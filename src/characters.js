...
const CHARACTERS = [
{
  ...
  specials:[
    { key:"bolt", check:(mb,b)=> (b & PUNCHES) && mb.qcf(),
      make:(b)=> sp({ id:"bolt_"+b, name:"Gale Bolt", startup:11, active:2, recovery:29,
        dmg:0, sfx:"fire", poses:[[[POSE.boltCoil,3],POSE.boltWind],
                [[POSE.boltPush,2],POSE.boltFire],
                [[POSE.boltFollow,5],[POSE.boltWind,5],[POSE.boltSettle,5],POSE.boltSet2]], hit:null,
    ...
    { key:"rise", check:(mb,b)=> (b & PUNCHES) && mb.dp(),
      make:(b)=> sp({ id:"rise_"+b, name:"Talon Rise", startup:3, active:12, recovery:24,
        ...
        poses:[[[POSE.riseCoil,2],POSE.riseWind],
                [[POSE.riseSmear,2],POSE.rise],
                [[POSE.rise,6],[POSE.riseFall,8],POSE.land]], hit:{x:2,y:-80,w:24,h:44} }) },
    { key:"heel", check:(mb,b)=> (b & KICKS) && mb.qcb(),
      make:(b)=> sp({ id:"heel_"+b, name:"Cyclone Heel", startup:7, active:16, recovery:20,
        ...
        poses:[[[POSE.kckCoil,3],POSE.spin1],
                [POSE.spin2,POSE.spin1,POSE.spin2,POSE.spin1],
                [[POSE.spin2,5],[POSE.land,5],POSE.kckSettle]], hit:{x:6,y:-58,w:30,h:36} }) }
  ],
  super:{ key:"tempest", name:"Tempest Bolt", check:(mb,b)=> (b & PUNCHES) && mb.doubleQcf(),
    make:()=> sp({ id:"super_tempest", name:"Tempest Bolt", type:"super",
      ...
      poses:[[[POSE.superRise,4],POSE.superHold],
              [[POSE.boltPush,2],POSE.boltFire],
              [[POSE.boltFollow,6],[POSE.boltWind,6],[POSE.boltSettle,6],POSE.boltSet2]], hit:null,
...
},
{
  ...
  specials:[
    { key:"rush", check:(mb,b)=> (b & PUNCHES) && mb.qcf(),
      make:(b)=> sp({ id:"rush_"+b, name:"Slab Rush", startup:12, active:14, recovery:26,
        ...
        poses:[[[POSE.fceCoilA,4],[POSE.fierceWind,4],POSE.fceCoilB],
                [[POSE.fceSmear,2],POSE.fierce],
                [[POSE.fceFollow,5],[POSE.straight,5],[POSE.fceSettle,5],POSE.fceSettle2]], hit:{x:8,y:-62,w:32,h:40} }) },
    { key:"clasp", check:(mb,b)=> (b & PUNCHES) && mb.hcf(),
      make:()=> sp({ id:"clasp", name:"Iron Clasp", type:"throw", level:"throw",
        ...
        poses:[[[POSE.jabCoil,2],POSE.grabReach],
                [[POSE.grabPull,3],POSE.grabHold],
                [[POSE.grabHold,5],POSE.grabReach]], hit:{x:8,y:-62,w:26,h:50} }) },
    { key:"anvil", check:(mb,b)=> (b & KICKS) && mb.qcb(),
      make:(b)=> sp({ id:"anvil_"+b, name:"Anvil Drop", level:"low", startup:14, active:8, recovery:22,
        ...
        poses:[[[POSE.riseCoil,3],POSE.jumpUp],
                [[POSE.swpSmear,2],POSE.sweep],
                [[POSE.swpFollow,5],[POSE.sweepWind,6],POSE.land]], hit:{x:-14,y:-16,w:50,h:16} }) }
  ],
  super:{ key:"wrecking", name:"Wrecking Line", check:(mb,b)=> (b & PUNCHES) && mb.doubleQcf(),
    make:()=> sp({ id:"super_wrecking", name:"Wrecking Line", type:"super",
      ...
      poses:[[[POSE.superRise,4],POSE.superHold],
              [[POSE.fceSmear,2],POSE.fierce],
              [[POSE.fceFollow,6],[POSE.straight,6],[POSE.fceSettle,6],POSE.fceSettle2]], hit:{x:6,y:-64,w:36,h:46} }) }
},
{
  ...
  specials:[
    { key:"lance", check:(mb,b)=> (b & PUNCHES) && mb.chargedFwd(), charge:"back",
      make:(b)=> sp({ id:"lance_"+b, name:"Static Lance", startup:8, active:2, recovery:26,
        dmg:0, sfx:"fire", poses:[[[POSE.lanceCoil,3],POSE.jabStart],
                [[POSE.lanceSmear,2],POSE.lance],
                [[POSE.lanceBack,5],[POSE.jabStart,4],[POSE.jabSettle,4],POSE.jabSettle2]], hit:null,
    ...
    { key:"fang", check:(mb,b)=> (b & KICKS) && mb.chargedUp(), charge:"down",
      make:(b)=> sp({ id:"fang_"+b, name:"Sky Fang", startup:4, active:14, recovery:22,
        ...
        air:true, sfx:"kick", poses:[[[POSE.fangCoil,2],POSE.riseWind],
                [[POSE.fangSmear,2],POSE.fang],
                [[POSE.fang,5],[POSE.fangBack,6],POSE.jumpFall]], hit:{x:0,y:-84,w:26,h:46} }) },
    { key:"razor", check:(mb,b)=> (b & KICKS) && mb.qcf(),
      make:(b)=> sp({ id:"razor_"+b, name:"Razor Spin", startup:6, active:14, recovery:19,
        ...
        poses:[[[POSE.kckCoil,3],POSE.spin2],
                [POSE.spin1,POSE.spin2,POSE.spin1,POSE.spin2],
                [[POSE.spin1,5],[POSE.land,5],POSE.kckSettle]], hit:{x:6,y:-52,w:32,h:34} }) }
  ],
  super:{ key:"circuit", name:"Circuit Break", check:(mb,b)=> (b & KICKS) && mb.doubleQcf(),
    make:()=> sp({ id:"super_circuit", name:"Circuit Break", type:"super",
      ...
      poses:[[[POSE.superRise,4],POSE.superHold],
              [POSE.spin1,POSE.spin2,POSE.spin1,POSE.spin2],
              [[POSE.spin1,6],[POSE.land,6],POSE.kckSettle]], hit:{x:4,y:-58,w:34,h:40} }) }
}
,
{
  ...
  specials:[
    ...
    { key:"slipper", check:(mb,b)=> (b & KICKS) && mb.dp(),
      make:(b)=> sp({ id:"slipper_"+b, name:"Slipper Rise", startup:4, active:12, recovery:26,
        ...
        air:true, sfx:"kick", poses:[[[POSE.fangCoil,2],POSE.riseWind],
                [[POSE.fangSmear,2],POSE.fang],
                [[POSE.fang,5],[POSE.fangBack,6],POSE.jumpFall]],
    ...
    { key:"slide", check:(mb,b)=> (b & KICKS) && mb.hcf(),
      make:(b)=> sp({ id:"slide_"+b, name:"Sock Slide", level:"low", startup:9, active:10,
        ...
        poses:[[[POSE.swpCoil,3],POSE.sweepWind],
                [[POSE.swpSmear,2],POSE.sweep],
                [[POSE.swpFollow,5],[POSE.sweepWind,6],POSE.crouch]], hit:{x:12,y:-12,w:32,h:12} }) }
  ],
  super:{ key:"service", name:"Full Service", check:(mb,b)=> (b & PUNCHES) && mb.doubleQcf(),
    make:()=> sp({ id:"super_service", name:"Full Service", type:"super",
      ...
      poses:[[[POSE.superRise,4],POSE.superHold],
              [[POSE.strSmear,2],POSE.straight],
              [[POSE.strFollow,6],[POSE.jab,6],[POSE.strSettle,6],POSE.strSettle2]], hit:{x:14,y:-58,w:38,h:26} }) }
}
];
